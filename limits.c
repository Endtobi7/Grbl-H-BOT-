...
void limits_go_home(uint8_t cycle_mask) 
{
  ...
  // Initialize
  bool simple_mode = (settings.homing_mode != 0);
  uint8_t n_cycle = simple_mode ? 1 : (2*N_HOMING_LOCATE_CYCLE+1);
  ...
  do {
    ...
    for (idx=0; idx<N_AXIS; idx++) {
      ...
      if (bit_istrue(cycle_mask,bit(idx))) {
        n_active_axis++;
        #ifdef COREXY
          // For CoreXY, zeroing the homed axis must be done in the A/B motor frame so that
          // the OTHER axis position is preserved. Otherwise the single-axis pull-off gets
          // mixed into both motors and the tool moves on a diagonal.
          if (idx == X_AXIS) {
